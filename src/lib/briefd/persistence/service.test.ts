...
import { unresolvedRowIds } from "./contracts";
...
function isActive(share: StoredShare, now: string): boolean {
  return !share.revokedAt && (!share.expiresAt || Date.parse(share.expiresAt) > Date.parse(now));
}
...
class MemoryRepository implements CampaignRepository {
  ...
  async replaceCampaign(record: StoredCampaign, expectedRevision: number, now: string) {
    ...
    if (!current || current.snapshot.revision !== expectedRevision) return "revision-conflict" as const;
    const hasActiveShare = [...this.shares.values()].some((share) => share.campaignId === record.snapshot.id && isActive(share, now));
    if (hasActiveShare && unresolvedRowIds(record.snapshot).length > 0) return "active-share-conflict" as const;
    this.campaigns.set(record.snapshot.id, structuredClone(record)); return "replaced" as const;
  }
  ...
  async createShare(share: StoredShare, now: string) {
    const campaign = this.campaigns.get(share.campaignId);
    if (!campaign) return "campaign-not-found" as const;
    if (isActive(share, now) && unresolvedRowIds(campaign.snapshot).length > 0) return "campaign-unresolved" as const;
    this.shares.set(share.id, structuredClone(share)); return "created" as const;
  }
...
describe("CampaignService", () => {
  ...
  it("keeps active shared links resolvable by rejecting unresolved updates", async () => {
    const repository = new MemoryRepository(); const service = new CampaignService(repository, fixedNow);
    const created = await service.createCampaign(draft);
    const campaignId = created.response.campaign.id;
    const share = await service.createShare(campaignId, created.ownerSecret);
    const unresolved = { ...campaignUpdateFromSnapshot(created.response.campaign), resolutions: [] };

    await expect(service.updateCampaign(unresolved, created.ownerSecret)).rejects.toMatchObject({
      status: 409,
    ...
    await expect(service.loadSharedCampaign(share.token)).resolves.toMatchObject({
      access: "view",
      campaign: { id: campaignId, revision: 1 },
    });
  });

  it("lists only shares that are active at the service clock", async () => {
    const repository = new MemoryRepository(); const service = new CampaignService(repository, fixedNow);
    const created = await service.createCampaign(draft); const campaignId = created.response.campaign.id;
    const active = await service.createShare(campaignId, created.ownerSecret, "2026-08-21T12:00:00.000Z");
    const baseShare = (await repository.listShares(campaignId))[0]!;
    await repository.createShare({ ...baseShare, id: "expired", tokenHash: "1".repeat(64), expiresAt: "2026-08-20T11:59:59.000Z" }, fixedNow().toISOString());
    await repository.createShare({ ...baseShare, id: "revoked", tokenHash: "2".repeat(64), revokedAt: "2026-08-20T11:00:00.000Z" }, fixedNow().toISOString());

    await expect(service.loadOwnerCampaign(campaignId, created.ownerSecret)).resolves.toMatchObject({
      shares: [{ id: active.id, expiresAt: "2026-08-21T12:00:00.000Z" }],
    });
  });
