...
export function ImportReview({ plan, matched, unmatched, onReparse, onResolve, onContinue, busy, error }: Props) {
  ...
  useEffect(() => {
    ...
    setHeaderRow(String(plan.headerRow));
  ...
  const sheetChanged = sheetName !== plan.sheetName;
  const headerChanged = Number(headerRow) !== plan.headerRow;
  const structureChanged = sheetChanged || headerChanged;
  const applySettings = () => {
    if (sheetChanged) return onReparse({ sheetName });
    if (headerChanged) return onReparse({ sheetName, headerRow: Number(headerRow) });
    return onReparse({ sheetName, headerRow: Number(headerRow), mapping });
  };
  return <div className="w-full max-w-5xl mx-auto flex flex-col gap-8">
    ...
    <section className="bg-light p-6 sm:p-8 grid gap-5"><h2 className="text-title font-bold">Workbook structure</h2>{plan.warnings.length > 0 && <ul className="grid gap-1 text-label text-black/70" aria-live="polite">{plan.warnings.map((warning) => <li key={warning}>• {warning}</li>)}</ul>}<div className="grid sm:grid-cols-2 gap-4"><label className="text-label font-bold">Sheet<select value={sheetName} onChange={(e) => setSheetName(e.target.value)} className="mt-1 block w-full border border-black/15 p-2 bg-white">{plan.availableSheets.map((sheet) => <option key={sheet}>{sheet}</option>)}</select></label><label className="text-label font-bold">Header row<input type="number" min="1" value={headerRow} onChange={(e) => setHeaderRow(e.target.value)} className="mt-1 block w-full border border-black/15 p-2 bg-white" /></label></div><div className="grid sm:grid-cols-3 gap-4">{FIELDS.map((field) => <label key={field.key} className="text-label font-bold">{field.label}{field.required ? " *" : ""}<select value={mapping[field.key] ?? ""} onChange={(e) => setColumn(field.key, e.target.value)} className="mt-1 block w-full border border-black/15 p-2 bg-white"><option value="">Not mapped</option>{plan.columns.map((column) => <option key={column.index} value={column.index}>{column.label} (column {column.index})</option>)}</select></label>)}</div>{error && <p className="text-label font-bold text-plum" role="alert">{error}</p>}<Button variant="soft" size="sm" disabled={busy || (!structureChanged && (!mapping.publisher || !mapping.format))} onClick={applySettings} className="justify-self-start">{busy ? "Applying…" : structureChanged ? "Detect structure" : "Apply mapping"}</Button></section>
    ...
    <div className="flex items-center justify-between gap-4"><p className="text-label text-black/60">Verified assignments cite the Brain source. Manual values remain visibly user-provided.</p><Button disabled={busy || unmatched.length > 0 || matched.length === 0} onClick={onContinue}>{busy ? "Saving campaign…" : "Save and open campaign"}</Button></div>
  </div>;
}
